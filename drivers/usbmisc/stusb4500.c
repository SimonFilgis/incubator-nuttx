...
#  define DUMPREG(priv, x) \
    do \
      { \
        ...
        if (ret < 0) \
          { \
            stusb4500_err("ERROR: Failed to read %s(0x%02X)\n", #x, (x)); \
          } \
        else \
          { \
            stusb4500_info("%s(0x%02X): 0x%02X\n", #x, (x), ret); \
          } \
      } \
    while(0)
#endif
                                  ...
                                  FAR uint8_t *dev_id, FAR uint8_t *dev_type)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to read device type\n");
      return -EIO;
    }
...
static int fusb303_clear_interrupts(FAR struct fusb303_dev_s *priv)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to clear interrupts\n");
      return -EIO;
    }
                         ...
                         struct fusb303_setup_s *setup)
{
  ...
  stusb4500_info("drp_tgl:%02X, host_curr:%02X\n"
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to enable chip\n");
      goto err_out;
    }
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to write mask register\n");
      goto err_out;
    }
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to write control register\n");
      goto err_out;
    }
                            ...
                            enum fusb303_mode_e mode)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to set portrole\n");
      ret = -EIO;
    }
                             ...
                             enum fusb303_manual_e state)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to set state\n");
      ret = -EIO;
    }
                               ...
                               FAR uint8_t *arg)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to read status\n");
      return -EIO;
    }
                                ...
                                FAR uint8_t *arg)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to read type\n");
      return -EIO;
    }
...
static int fusb303_reset(FAR struct fusb303_dev_s *priv)
{
  ...
  if (ret < 0)
    {
      stusb4500_err("ERROR: Failed to reset chip\n");
      ret = -EIO;
    }
